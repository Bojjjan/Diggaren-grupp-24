...
class TopListUpdater:
    def __init__(self):
        ...
        self.db = DatabaseManager(db_host, db_user, db_pass, db_name)
    ...
    def update_top_list(self):
        while not self.stop_event.is_set():
            ...
            for track in sr_channels:
                ...
                if not self.db.get_channel_by_id(track.channel_id):
                    print("TopListUpdater: Adding channel ", track.channel_id, " added to DB.")

                    self.db.add_channel_with_id(track.channel_id, track.channel_name)

                if not self.db.get_track_by_id(track.spotify_id):
                    print(f"TopListUpdater: Adding track '{track.song_title}' by '{track.artist_name}'to "
                          f"'Tracks' table.")

                    self.db.add_track(spotify_id=track.spotify_id, title=track.song_title, artist=track.artist_name)

                if self.db.get_latest_track_from_channel(track.channel_id) != track.spotify_id:
                    print(f"TopListUpdater: Adding track '{track.song_title}' by '{track.artist_name}'to "
                          f"'PlayHistory' table.")

                    self.db.add_play_history(spotify_id=track.spotify_id, channel_id=track.channel_id)
